...
int spacing[SB_END]= {4,		8,		4,		8,		4,		8,		4,		4,		4,		8};
...
void InitSpeedBarList(void)
{
	...
	text[i++] = GetAmiFTPString(MW_Parent);//"Parent";
	text[i++] = GetAmiFTPString(MW_Reload);
	text[i++] = GetAmiFTPString(MW_Get);
	text[i++] = GetAmiFTPString(MW_Put);
	text[i++] = GetAmiFTPString(MW_CreateDir);//"Create Dir";
	text[i++] = GetAmiFTPString(MW_Delete);//"Delete";
	text[i++] = GetAmiFTPString(MW_Rename);//"Rename";
	text[i++] = GetAmiFTPString(MW_View);
	text[i++] = GetAmiFTPString(MW_Settings);//"Settings";
	text[i++] = GetAmiFTPString(MW_Disconnect);

	speedImages[SB_CONNECT]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
	...
	speedImages[SB_DISCONNECT]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
			BITMAP_Screen, Screen,
	...
	speedImages[SB_PARENT]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
			BITMAP_Screen, Screen,
	...
	speedImages[SB_RELOAD]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_DOWNLOAD]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_UPLOAD]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_MKDIR]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_DELETE]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_RENAME]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_SHOW]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
	...
	speedImages[SB_SETTINGS]= (struct Image *)NewObject(BitMapClass, NULL, //"bitmap.image",
		   BITMAP_Screen, Screen,
...
Remove(node);
	...
	for (int i=0; i < SB_END+1; i++) { // +1 -> SB_DISCONNECT=SB_END
		DisposeObject( (Object *)speedImages[i] );
	}
...
int HandleSpeedBar(int button)
{
	...
	{
		case SB_CONNECT:
			if (connected) Disconnect_clicked();
			else Connect_clicked();
		break;
		...
		case SB_SHOW:
			if (FileList && connected) View_clicked(CurrentState.ADTMode);
		break;
